...
void QuaActivF(int ci, int yi, int xi,float in[16][16][32],
               int out[16][16][32])
{
  for(int c=0; c<ci; c++){
    for(int y=0; y<yi; y++){
...
void Affine(int xi,int ci, ulong in[16], float f[10][512], float out[10])
{
  for(int c=0; c<ci; c++){
    out[c] = 0;
    for(int x=0; x<xi; x++){
      int xx = (in[x/32]>>((x%32)*2)) & 3;
      out[c] += f[c][x] * (xx*(-2)+3);
    }
  }
}
...
int main(int argc, char **argv, char **env) {
  ...
  ulong activ1bin[16][16];

  ulong activ2bin[8][8];

  ulong activ3bin[4][4][2];
    ...
    for(int y=0; y<16; y++){
      for(int x=0; x<16; x++){
        ...
        for(int c=0; c<32; c=c+1){
          if(activ1out[y][x][c]==-3){
            activ1bin[y][x] |= (ulong)3<<(c*2);
          } else if(activ1out[y][x][c]==-1){
            activ1bin[y][x] |= (ulong)2<<(c*2);
          } else if(activ1out[y][x][c]==1){
            activ1bin[y][x] |= (ulong)1<<(c*2);
          }
        }
      }
    }
            ...
            for(int fy=0; fy<3; fy=fy+1){
              for(int fx=0; fx<3; fx=fx+1){
                ...
                if(((py+fy)==0)|((py+fy)==17)|((px+fx)==0)|((px+fx)==17)){
                  verilator_top->data=0x5555555555555555;
                }else{
  ...
  for(int y=0; y<4; y++){
    for(int x=0; x<4; x++){
      printf("%016lx, %016lx, ",activ3bin[y][x][0],activ3bin[y][x][1]);
    }
    printf("\n");
  }
